...
- 単価は 2026 年 7 月時点（AgentCore Runtime のみ 2026 年 9 月時点）
- Lambda のリクエスト課金と無料枠、EC2 の EBS 代、AgentCore Runtime のストレージ代・データ転送代は含まない
...
| Bedrock AgentCore Runtime (microVMs) | 25,322 | ███████████████ | 2.53 倍 | 2.20 倍 |
| CodeBuild | 29,784 | ██████████████████ | 2.98 倍 | 2.59 倍 |

※ AgentCore Runtime (microVMs) は 2 vCPU / 4 GB を常時消費した場合の上限値。CPU は実消費分のみの課金で I/O 待ち中は課金されないため、待ち時間の割合が大きいエージェント処理では大幅に下がる。
...
| Bedrock AgentCore Runtime (microVMs) | - | 2 vCPU / 4 GB 消費時 | 0.2168 | 秒（最低 1 秒） | 同期 15 分 / ストリーミング 60 分 / 非同期 8 時間 |
| CodeBuild | arm1.small | 2 vCPU / 3 GB | 0.255 | 分 | 最大 36 時間 / ビルド |

※ AgentCore Runtime (microVMs) はリソースを事前に選択せず、CPU は実消費分、メモリはその秒までのピーク消費分（最低 128 MB、システムオーバーヘッドを含む）で課金される。単価は $0.0895 / vCPU 時間 + $0.00945 / GB 時間 なので、2 vCPU / 4 GB 相当の消費は `2 × 0.0895 + 4 × 0.00945 = 0.2168 USD/時`。表のタイムアウトは 1 回の実行あたりの上限で、いずれも変更不可。セッション自体はアイドル 15 分・最大 8 時間で終了し、こちらは `LifecycleConfiguration` で変更できる。

※ AgentCore Runtime にはこの microVMs のほかに、EC2 インスタンス料金 + 管理費で課金される Instances タイプもある。また、モデル推論費用（Bedrock などの料金）は別途かかる。

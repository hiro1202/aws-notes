...
※ 比較のため 1 ヶ月（730 時間）常時稼働に換算しているが、Lambda は 1 実行あたり最大 15 分、CodeBuild は 1 ビルドあたり最大 36 時間のタイムアウト制限があるため、実際にはこの条件では動かせない。
...
|---|---|---|---:|
| EC2 | c7g.large | 2 vCPU / 4 GiB | 0.0910 |
| Fargate (ECS) | - | 2 vCPU / 4 GB | 0.09858 |
| Lambda | メモリ 4 GB 設定 | 4 GB（CPU はメモリに比例して割り当てられる） | 0.19200 |

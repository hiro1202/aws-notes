...
※ 常時稼働換算は比較用。Lambda と CodeBuild にはタイムアウト制限がある（詳細比較を参照）。

---

## 詳細比較

| サービス | 比較対象 | スペック | 単価 (USD/時) | タイムアウト |
|---|---|---|---:|---|
| EC2 | c7g.large | 2 vCPU / 4 GiB | 0.0910 | なし |
| Fargate (ECS) | - | 2 vCPU / 4 GB | 0.09858 | なし |
| Lambda | メモリ 4 GB 設定 | 4 GB（CPU はメモリに比例して割り当てられる） | 0.19200 | 最大 15 分 / 実行 |
| CodeBuild | arm1.small | 2 vCPU / 3 GB | 0.255 | 最大 36 時間 / ビルド |
